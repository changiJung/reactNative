...
const GET_BookShelves = 'User/GET_BookShelves';
// 리스트 가져오기 성공 --> loading값을 false로, result는 채우고, error는 false로 처리
const GET_BookShelves_SUCCESS = 'User/GET_BookShelves_SUCCESS';
// 리스트 가져오기 실패 --> loading값을 false로, result는 비우고, error는 true로 처리
const GET_BookShelves_FAILURE = 'User/GET_BookShelves_FAILURE';

/** 3) 액션 생성 함수 - 액션 객체를 만들어서 리턴한다. */
// ReduxThunk에 의한 비동기 함수안에서 Ajax 처리 후, 결과를 반환하기 위해 이 함수들을 dispatch한다.
export const getBookShelvesAction          = createAction(GET_BookShelves);
export const getBookShelvesSuccessAction   = createAction(GET_BookShelves_SUCCESS);
export const getBookShelvesFailureAction   = createAction(GET_BookShelves_FAILURE);
...
export default handleActions(
    {
        // 리스트 가져오기 시작 --> loading값을 true로, result는 비우고, error는 false로 처리
        [GET_BookShelves]: (state = initialState, action) => {
            return {
                ...state,
                loading: true,
                result: [],
        ...
        // 리스트 가져오기 성공 --> loading값을 false로, result는 채우고, error는 false로 처리
        [GET_BookShelves_SUCCESS]: (state = initialState, action) => {
            return {
                ...state,
                loading: false,
        ...
        // 리스트 가져오기 실패 --> loading값을 false로, result는 비우고, error는 true로 처리
        [GET_BookShelves_FAILURE]: (state = initialState, action) => {
            return {
                ...state,
                loading: false,
                result: [],
...
// 실제 동작할 함수 하나만 정의.
// 이 안에서 상태가 변경되도록 구성
export const BookShelvesAsync = (token) => async dispatch => {
    // 리스트 가져오기 시작 --> loading값을 true로, result는 비우고, error는 false로 처리
    dispatch(getBookShelvesAction());
    ...
    console.log('북셀브인포 ' + token);        

        const response = fetch('http://124.49.227.243:8083/bookShelves/book', {
            method: 'GET',
            ...
            .then((json) =>
            dispatch(getBookShelvesSuccessAction({result: json.token}))
        ...
        // dispatch(getBookShelvesSuccessAction({result: response.data}));
    } catch (e) {
        /** 리스트 가져오기 실패 --> loading값을 false로, result는 비우고, error는 true로 처리 */
        // 에러 내용을 로그에 출력해 보자!!!
        console.group("에러내용");
        ...
        console.groupEnd();
        // 여기서 전달하는 파라미터가 action 함수 안에서 `action.payload`가 된다.
        dispatch(getBookShelvesFailureAction({error: '유저정보를 가지고 오는데 실패했습니다.'}));
    }
};