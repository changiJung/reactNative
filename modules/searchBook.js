...
const GET_LIST = 'Book/GET_LIST';
// 리스트 가져오기 성공 --> loading값을 false로, result는 채우고, error는 false로 처리
const GET_LIST_SUCCESS = 'Book/GET_LIST_SUCCESS';
// 리스트 가져오기 실패 --> loading값을 false로, result는 비우고, error는 true로 처리
const GET_LIST_FAILURE = 'Book/GET_LIST_FAILURE';


const url = 'http://124.49.227.243:8083/search/book'
...
// 실제 동작할 함수 하나만 정의.
// 이 안에서 상태가 변경되도록 구성
export const bookListAsync = (searchText, searchType, searchPage) => async dispatch => {
    // 리스트 가져오기 시작 --> loading값을 true로, result는 비우고, error는 false로 처리
    ...
    try {
        ...
        dispatch(getListSuccessAction({result: response.data}));
    } catch (e) {
        /** 리스트 가져오기 실패 --> loading값을 false로, result는 비우고, error는 true로 처리 */
        // 에러 내용을 로그에 출력해 보자!!!
        console.group("에러내용");
